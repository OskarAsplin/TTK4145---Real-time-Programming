...
func ElevInit() {
	...
	for floor := 0; floor < NUMFLOORS; floor++ {
		for button := 0; button < 3; button++ {
			ElevSetButtonLight(button, floor, false)
		}
	}
	ElevSetFloorIndicator(0)
	ElevSetDoorLight(false)
}

func ElevSetButtonLight(button int, floor int, value bool) { //We want assert?
...
func ElevSetDoorLight(value bool) {
	if value == true {
